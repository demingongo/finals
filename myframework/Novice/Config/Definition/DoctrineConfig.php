<?php
namespace Novice\Config\Definition;

use Symfony\Component\Config\Definition\Builder\ArrayNodeDefinition;
use Symfony\Component\Config\Definition\ConfigurationInterface;
use Symfony\Component\Config\Definition\Builder\TreeBuilder;

class DoctrineConfig implements ConfigurationInterface
{
	private $debug;
			...
			->children()
				...
				//	->children()
						...
						->scalarNode('driver')
							->isRequired()
							->validate()
							->ifNotInArray(array('mysql', 'sqlite', 'mssql','pdo_mysql'))
    ...
    private function getDbalConnectionsNode()
    {
        ...
        $connectionNode
            ...
            ->children()
				...
				->scalarNode('serverVersion')->end()
                ->scalarNode('driverClass')->end()
                ->scalarNode('wrapperClass')->end()
                ->booleanNode('keepSlave')->end()
            ...
            ->end()
			->beforeNormalization()
                ->ifTrue(function($v) {return !isset($v['serverVersion']) && isset($v['server_version']);})
                ->then(function($v) {
                    $v['serverVersion'] = $v['server_version'];
                    unset($v['server_version']);

                    return $v;
                })
            ->end()
			->beforeNormalization()
                ->ifTrue(function($v) {return !isset($v['driverClass']) && isset($v['driver_class']);})
                ->then(function($v) {
                    $v['driverClass'] = $v['driver_class'];
                    unset($v['driver_class']);

                    return $v;
                })
            ->end()
			->beforeNormalization()
                ->ifTrue(function($v) {return !isset($v['driverOptions']) && isset($v['options']);})
                ->then(function($v) {
                    $v['driverOptions'] = $v['options'];
                    unset($v['options']);
            ...
            ->end()
			->beforeNormalization()
                ->ifTrue(function($v) {return !isset($v['wrapperClass']) && isset($v['wrapper_class']);})
                ->then(function($v) {
                    $v['wrapperClass'] = $v['wrapper_class'];
                    unset($v['wrapper_class']);

                    return $v;
                })
            ->end()
			->beforeNormalization()
                ->ifTrue(function($v) {return !isset($v['keepSlave']) && isset($v['keep_slave']);})
                ->then(function($v) {
                    $v['keepSlave'] = $v['keep_slave'];
                    unset($v['keep_slave']);
            ...
            ->children()
                ->arrayNode('orm')
                    ...
                    ->children()
                        ...
                        ->scalarNode('auto_generate_proxy_classes')->defaultValue($this->debug)
                            ...
                            ->end()
                        ->end()
                        ->scalarNode('proxy_dir')->defaultValue('%app.cache_dir%/doctrine/orm/Proxies')->end()
