...
class ImportMappingDoctrineCommand extends DoctrineCommand
{
    ...
    protected function configure()
    {
        $this
            ...
            ->addArgument('module', InputArgument::REQUIRED, 'The module to import the mapping information to')
...
<info>php %command.full_name% "MyCustomModule" xml</info>

You can also optionally specify which entity manager to import from with the
<info>--em</info> option:

<info>php %command.full_name% "MyCustomModule" xml --em=default</info>

If you don't want to map every entity that can be found in the database, use the
<info>--filter</info> option. It will try to match the targeted mapped entity with the
provided pattern string.

<info>php %command.full_name% "MyCustomModule" xml --filter=MyMatchedEntity</info>

Use the <info>--force</info> option, if you want to override existing mapping files:

<info>php %command.full_name% "MyCustomModule" xml --force</info>
EOT
        );
    }
    ...
    {
        $module = $this->getApplication()->getApp()->getModule($input->getArgument('module'));

        $destPath = $module->getPath();
        ...
        if ($metadata) {
            ...
            foreach ($metadata as $class) {
                ...
                $class->name = $module->getNamespace().'\\Entity\\'.$className;
                if ('annotation' === $type) {
