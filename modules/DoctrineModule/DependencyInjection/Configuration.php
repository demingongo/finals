...
class Configuration implements ConfigurationInterface
{
    ...
    public function getConfigTreeBuilder()
    {
        ...
        $rootNode = $treeBuilder->root('managers');

		$generationModes = $this->getAutoGenerateModes();

		$rootNode
			->children()
			 ->booleanNode('activate')->defaultTrue()->end()
			 ->end()
			->beforeNormalization()
                    ->ifTrue(function ($v) { return is_array($v) && !array_key_exists('connections', $v) && !array_key_exists('connection', $v); })
                    ->then(function ($v) {
                        // Key that should not be rewritten to the connection config
                        $excludedKeys = array('default_connection' => true , 'activate' => true, 
							'auto_generate_proxy_classes' => true, 'proxy_dir' => true, 'proxy_namespace' => true, 
							'class_metadata_factory_name' => true , 'default_repository_class' => true, 'naming_strategy' => true, 
							'mappings' => true, 'dql' => true, 'hydrators' => true,
							);
                        $connection = array();
             ...
             ->children()
				 ...
				 ->scalarNode('naming_strategy')->defaultValue('managers.naming_strategy.default')->end()
				 /*->arrayNode('dql')
