...
<div class="col-lg-8">
	...
	<div id="collapse-tools" class="col-lg-8 collapse subhead-collapse">
		...
		<button type="submit" name="submit[]" class="btn btn-outline btn-default itemAction to-xs" value="valid">
		 <span class="fa fa-check-circle text-success"></span> Valid
        </button>
...
<table id="tab" class="table table-striped table-hover ">
  ...
  <tbody>
    ...
    <td>
	  	<input type="checkbox" id="cb{$smarty.foreach.requests.index}" name="cid[]" value="{$ureq.id}" />
	</td>
	<td>
	<input type="image" src="{statut statut=$ureq.isPublished srconly=true}" class="btn btn-outline btn-default" name="submit[]" onclick="formTache('valid','cb{$smarty.foreach.requests.index}')" value="valid"  />
	</td>
	<td>
